    ...
    for epoch in range(start_epoch, config['num_epochs']):
        ...
        with torch.no_grad():
            ...
            for batch in val_progress_bar:
                ...
                all_targets.append(kcats.squeeze(-1).cpu().numpy())
        ...
        # Save best model and check early stopping based on Spearman
        if val_spearman > best_spearman:
            best_spearman = val_spearman
            patience_counter = 0
            torch.save(model.state_dict(), os.path.join(output_dir, 'best_model.pth'))
            logger.info(f"Best model saved with Spearman: {best_spearman:.4f}")
        else:
            patience_counter += 1
            if patience_counter >= patience:
                logger.info(f"Early stopping triggered at epoch {epoch+1} (no improvement in Spearman)")
                break
