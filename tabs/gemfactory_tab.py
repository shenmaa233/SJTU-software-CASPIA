...
def search_bigg_models(search_query: str):
    ...
    if not search_query:
        ...
        filtered = [
            m for m in models
            if search_lower in m.get("bigg_id", "").lower() 
            or search_lower in m.get("organism", "").lower()
        ]
                ...
                try:
                    ...
                    if protein_seq:
                        ...
                        for i in range(0, len(protein_seq), 60):
                            fasta_file.write(f"{protein_seq[i:i+60]}\n")
                        protein_count += 1
...
def start_ko_oe_analysis(selected_model, analysis_type, target_reaction, 
                         knockout_threshold, oe_fold_changes_text, _sid):
...
def ko_oe_analysis_tab():
    ...
    run_btn.click(
        ...
        inputs=[model_dropdown, analysis_type_dropdown, target_reaction_box,
                knockout_threshold_slider, oe_fold_changes_box, task_id_box],
...
def gem_analysis_tab():
    ...
    run_btn.click(
        ...
        inputs=[model_dropdown, algorithm_dropdown, obj_textbox, 
                substrate_textbox, concentration_slider, task_id_box],
        outputs=[task_id_box, status_box, logs_box, result_summary]
    )
